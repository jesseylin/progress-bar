<script lang="ts">
	...
	$effect(() => {
		const match = presets.findIndex(([, h, m, s]) => h === hh && m === mm && s === ss);
...
<main class="min-h-screen bg-neutral-950 text-neutral-100 flex items-center justify-center p-6">
	<div class="w-full max-w-xl rounded-2xl bg-neutral-900/60 ring-1 ring-white/10 shadow-2xl p-5 space-y-4">
		<div class="h-20 w-full rounded-2xl bg-neutral-800 overflow-hidden ring-1 ring-white/5">
			<div
		...
		<div class="flex justify-center">
			...
			>
				<svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="5 8 10 13 15 8"></polyline></svg>
			</button>
		</div>
		...
		<div class="text-center">
			<div class="text-5xl font-semibold tabular-nums tracking-tight">
		...
		<div class="flex justify-center gap-2 text-xs text-neutral-500">
			{#each presets as [label, h, m, s], i}
				<button
					onclick={() => { hh = h; mm = m; ss = s; selectedPreset = i; }}
					disabled={startedAt !== null}
					class="px-3 py-1 rounded-full ring-1 transition-colors disabled:opacity-30 {selectedPreset === i ? 'bg-sky-500/20 text-sky-300 ring-sky-400/50' : 'ring-white/10 hover:bg-neutral-800'}"
				>{label}</button>
			{/each}
		</div>
	</div>
